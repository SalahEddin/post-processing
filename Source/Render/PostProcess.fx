...
{
	float3 ppCol = SceneTexture.Sample(PointSample, ppIn.UV);
	int halfBlurRange = BlurRange / 2;

	float weightTotal = 0;

	float3 pointCol = float3(0.f, 0.f, 0.f);
	...
	for (int i = 0; i < BlurRange; i++)
	{
		...
		float weight = ((float[4]) (GaussianFilter[i / 4]))[i % 4];
		weightTotal += weight;
		pointCol = SceneTexture.Sample(PointSample, ppIn.UV + float2(xOffset, 0.0f)) * weight;

		// check colour lumiance
		if ((pointCol.x + pointCol.y + pointCol.z) > BloomThreshold)
		{
			pointCol.xyz += 2.0f;
		}
	...
	if ((ppCol.r + ppCol.g + ppCol.b) < BloomThreshold - 0.1f)
	{
		ppCol = SceneTexture.Sample(PointSample, ppIn.UV);
...
float4 PPBloomShaderV(PS_POSTPROCESS_INPUT ppIn) : SV_Target
{
	...
	for (int i = 0; i < BlurRange; i++)
	{
		...
		float weight = ((float[4]) (GaussianFilter[i / 4]))[i % 4];
		weightTotal += weight;
		pointCol = SceneTexture.Sample(PointSample, ppIn.UV + float2(0.0f, xOffset)) * weight;
		// check colour lumiance
		if ((pointCol.x + pointCol.y + pointCol.z) > BloomThreshold)
		{
			pointCol.xyz += 2.0f;
